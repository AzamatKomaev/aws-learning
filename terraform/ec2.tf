resource "aws_key_pair" "nginx_lb_demo" {
  key_name   = "nginx-lb-demo"
...
# LB Testing
resource "aws_instance" "nginx_lb_demo" {
  count = 0
  ...
  key_name                    = aws_key_pair.nginx_lb_demo.key_name
  private_ip                  = var.nginx_lb_demo_private_ips[count.index]
  subnet_id                   = aws_default_subnet.default_a.id
  tenancy                     = "default"
  vpc_security_group_ids      = [aws_default_security_group.default.id]
  capacity_reservation_specification {
